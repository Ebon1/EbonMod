            ...
            if (verlet != null)
            {
                ...
                for (int i = 0; i < (NPC.scale > 0.55f ? 5 : 2); i++)
                {
                    if (extraVerlets[i] != null)
                    {
                        ...
                        extraVerlets[i].Draw(spriteBatch, Texture + "_Tex", textureVariation: true, maxVariants: 6, variantSeed: texNum);

                        if (Main.LocalPlayer.HasBuff(BuffID.Hunter) && !NPC.IsABestiaryIconDummy)
                            extraVerlets[i].Draw(spriteBatch, Texture + "_Tex", textureVariation: true, maxVariants: 6, variantSeed: texNum, useColor: true, color: NPC.HunterPotionColor());
                    }
                }
                verlet.Draw(spriteBatch, Texture + "_Tex", textureVariation: true, maxVariants: 6, variantSeed: texNum);

                if (Main.LocalPlayer.HasBuff(BuffID.Hunter) && !NPC.IsABestiaryIconDummy)
                    verlet.Draw(spriteBatch, Texture + "_Tex", textureVariation: true, maxVariants: 6, variantSeed: texNum, useColor: true, color: NPC.HunterPotionColor());
                if (NPC.scale > 0.55f)
                    for (int i = 5; i < 7; i++)
                    {
                        if (extraVerlets[i] != null)
