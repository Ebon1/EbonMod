...
namespace EbonianMod.Items.Weapons.Ranged
{
    public class MagmaBow : ModItem
    {
        public override void SetDefaults()
        {
            ...
            Item.shootSpeed = 1f;
            Item.shoot = ModContent.ProjectileType<MagmaBowP>();
        ...
        {
            Projectile.NewProjectile(source, position, velocity, ModContent.ProjectileType<MagmaBowP>(), damage, knockback, player.whoAmI);
            return false;
        }
    ...
    public class MagmaBowP : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            if (Projectile.ai[2] > 30)
                off = 0;
            Utils.DrawLine(Main.spriteBatch, Projectile.Center - new Vector2(8, 20).RotatedBy(Projectile.rotation), Projectile.Center - new Vector2(8 + Projectile.ai[2], -off).RotatedBy(Projectile.rotation), new Color(162, 44, 31), new Color(162, 44, 31), 2);
            Utils.DrawLine(Main.spriteBatch, Projectile.Center - new Vector2(10, -20).RotatedBy(Projectile.rotation), Projectile.Center - new Vector2(8 + off + Projectile.ai[2], off).RotatedBy(Projectile.rotation), new Color(162, 44, 31), new Color(162, 44, 31), 2);
            return true;
        }
        ...
        {
            Texture2D tex = Helper.GetTexture("Projectiles/Friendly/Underworld/MagmaArrow");
            for (int i = -2; i < 3; i++)
            {
                if (i == 0)
                    continue;
                Main.spriteBatch.Draw(tex, Projectile.Center + Vector2.Lerp(new Vector2(20, 0).RotatedBy(Projectile.rotation), Vector2.Zero, alpha[i + 2]) - Main.screenPosition, null, Color.White * alpha[i + 2], Projectile.rotation + (i * 0.25f) - MathHelper.PiOver2, tex.Size() / 2, 1, SpriteEffects.None, 0);
            }
        ...
        public override void AI()
        {
            ...
            if (Projectile.timeLeft > 25)
            {
                ...
                for (int i = 0; i < 1 + MathHelper.Clamp((-Projectile.timeLeft + maxTime) / 5, 0, 4); i++)
                {
            ...
            else
            {
                if (Projectile.timeLeft == 24)
                {
                    ...
                    for (int i = -2; i < 3; i++)
                    {
                        Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, Projectile.velocity.RotatedBy(i * 0.25f) * 20, ModContent.ProjectileType<MagmaArrow>(), Projectile.damage, Projectile.knockBack, Projectile.owner, -i);
                    }
                    for (int i = 0; i < 5; i++)
                        alpha[i] = 0;
                }
