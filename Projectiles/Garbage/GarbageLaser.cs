...
namespace EbonianMod.Projectiles.Garbage
{
    public class GarbageLaser1 : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Texture2D texture3 = Helper.GetExtraTexture("Ex1");
            Texture2D texture4 = Helper.GetExtraTexture("cone5");
            float progress = Utils.GetLerpValue(0, maxTime, Projectile.timeLeft);
            ...
            for (float i = 0; i < 1; i += 0.005f)
            {
                ...
                col = Color.Lerp(Color.DarkRed, Color.Red, i * 9) * 0.1f;
                vertices4.Add(Helper.AsVertex(start + Projectile.velocity.ToRotation().ToRotationVector2() * 2500 * 0.4f * i + new Vector2(250, 0).RotatedBy(rot + MathHelper.PiOver2) * i_progress, new Vector2(i, 1), col * Projectile.scale));
                vertices4.Add(Helper.AsVertex(start + Projectile.velocity.ToRotation().ToRotationVector2() * 2500 * 0.4f * i + new Vector2(250, 0).RotatedBy(rot - MathHelper.PiOver2) * i_progress, new Vector2(i, 0), col * Projectile.scale));
            ...
            if (vertices.Count >= 3 && vertices2.Count >= 3 && vertices3.Count >= 3 && vertices4.Count >= 3)
            {
                for (int i = 0; i < 4; i++)
                {
                    ...
                    Helper.DrawTexturedPrimitives(vertices4.ToArray(), PrimitiveType.TriangleStrip, texture4, false);
                }
            }
