...
namespace EbonianMod.Projectiles.ArchmageX
{
    public class XAmethystShard : ModProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            for (int i = 0; i < 5; i++)
            {
                Dust.NewDustPerfect(Projectile.Center, ModContent.DustType<SparkleDust>(), Main.rand.NextVector2Circular(3, 3), 0, Color.DarkOrchid, Main.rand.NextFloat(0.05f, 0.175f));
                Dust.NewDustPerfect(Projectile.Center, ModContent.DustType<LineDustFollowPoint>(), Main.rand.NextVector2Circular(5, 5), 0, Color.DarkOrchid, Main.rand.NextFloat(0.05f, 0.24f));
            }
